...
// Label Cropper
let cropPdfFile=null,cropPdfUrl=null;
function setCropFile(file){
 if(!file||file.type!=='application/pdf'){show('Valid PDF file select karein.',true);return;}
 cropPdfFile=file;if(cropPdfUrl)URL.revokeObjectURL(cropPdfUrl);cropPdfUrl=URL.createObjectURL(file);
 const info=$('#cropFileInfo');if(info)info.textContent=`${file.name} • ${(file.size/1024/1024).toFixed(2)} MB`;
 const status=$('#cropStatus');if(status)status.textContent='PDF ready. Preview ya Crop & Download use karein.';
}
if($('#cropPdfInput'))$('#cropPdfInput').onchange=e=>setCropFile(e.target.files[0]);
if($('#cropDrop')){const d=$('#cropDrop');['dragenter','dragover'].forEach(t=>d.addEventListener(t,e=>{e.preventDefault();d.classList.add('drag')}));['dragleave','drop'].forEach(t=>d.addEventListener(t,e=>{e.preventDefault();d.classList.remove('drag')}));d.addEventListener('drop',e=>setCropFile(e.dataTransfer.files[0]));}
if($('#cropPreviewBtn'))$('#cropPreviewBtn').onclick=()=>{if(!cropPdfUrl)return show('Pehle label PDF select karein.',true);$('#cropPreview').src=cropPdfUrl;$('#cropPreview').classList.remove('hidden');$('#cropPreviewEmpty').classList.add('hidden');};
async function robustCropPdf(file){
 if(!globalThis.PDFLib?.PDFDocument)throw new Error('Bundled PDF engine unavailable. pdf-lib.min.js bundle required.');
 ...
 const outputSize=$('#cropPageSize')?.value||'4x6';
 const customTop=Math.max(0,+$('#cropTop').value||0),customRight=Math.max(0,+$('#cropRight').value||0),customBottom=Math.max(0,+$('#cropBottom').value||0),customLeft=Math.max(0,+$('#cropLeft').value||0);
 const preset=$('#cropPreset')?.value||'flipkart';
 ...
 for(const sourcePage of source.getPages()){
   ...
   if(preset==='flipkart'){
     // Exact Flipkart rule copied from the supplied production cropper:
     // portrait {x:.32,y:.548,w:.36,h:.418}; landscape {x:.02,y:.05,w:.96,h:.90}
     const r=portrait?{x:.32,y:.548,w:.36,h:.418}:{x:.02,y:.05,w:.96,h:.90};
     left=pw*r.x;bottom=ph*r.y;right=left+pw*r.w;top=bottom+ph*r.h;
   }else{
     left=customLeft;bottom=customBottom;right=pw-customRight;top=ph-customTop;
   }
   if(right-left<=10||top-bottom<=10)throw new Error('Crop area invalid hai. Margins/preset check karein.');
   const embedded=await output.embedPage(sourcePage,{left,bottom,right,top});
   const cropW=right-left,cropH=top-bottom;
   const target=sizes[outputSize]||[cropW,cropH];
   const page=output.addPage(target);
   const pad=outputSize==='original'?0:8;
   const scale=Math.min((target[0]-pad*2)/cropW,(target[1]-pad*2)/cropH);
   const drawW=cropW*scale,drawH=cropH*scale;
   page.drawPage(embedded,{x:(target[0]-drawW)/2,y:(target[1]-drawH)/2,width:drawW,height:drawH});
 }
 output.setTitle('Flipkart cropped shipping labels');
 output.setProducer('Flipkart Analytics & Tools');
 return new Blob([await output.save({useObjectStreams:true,addDefaultPage:false})],{type:'application/pdf'});
}
if($('#cropPreset'))$('#cropPreset').onchange=()=>{const v=$('#cropPreset').value;if(v==='flipkart'){[['cropTop',0],['cropRight',0],['cropBottom',0],['cropLeft',0]].forEach(([i,n])=>$('#'+i).value=n)}else if(v==='tight'){[['cropTop',8],['cropRight',8],['cropBottom',8],['cropLeft',8]].forEach(([i,n])=>$('#'+i).value=n)}};
if($('#cropProcessBtn'))$('#cropProcessBtn').onclick=async()=>{if(!cropPdfFile)return show('Pehle label PDF select karein.',true);const btn=$('#cropProcessBtn'),status=$('#cropStatus');btn.disabled=true;status.textContent='PDF crop process ho raha hai…';try{const blob=await robustCropPdf(cropPdfFile);const u=URL.createObjectURL(blob);chrome.downloads.download({url:u,filename:cropPdfFile.name.replace(/\.pdf$/i,'')+'_cropped.pdf',saveAs:true},()=>setTimeout(()=>URL.revokeObjectURL(u),60000));status.textContent='Cropped PDF created with page-level PDF processing. Preview the downloaded file before bulk use.'}catch(err){status.textContent=err.message+' Original PDF preview karke custom crop values adjust karein.';show(err.message,true)}finally{btn.disabled=false}};
